...
* **스키마 모듈화**: `nameField`, `emailField` 등 최소 단위 스키마를 정의하고 이를 조합하여 로그인/회원가입/문의하기 스키마를 구성했습니다.
* **비즈니스 로직 응집**: 공백 체크, 특수문자 조합, 비밀번호 일치 확인 등 복잡한 요구사항을 Zod의 `refine`과 `regex`로 선언적으로 처리하여 가독성을 높였습니다.

### 2. 공통 컴포넌트 설계
* **`Input`**: Zod 에러 메시지를 Props로 받아 실시간 피드백을 제공하며, Password Toggle 기능을 내장했습니다.
* **`Button`**: 다양한 Variants와 로딩 상태를 통합 관리하여 서비스 전반의 인터랙션 일관성을 확보했습니다.

---

## 배포 및 실행 방법 (Deployment & Run)

구현된 서비스는 **Vercel**을 통해 배포되었으며, 아래 링크에서 즉시 확인하실 수 있습니다.
